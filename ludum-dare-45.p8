...
-->8 MENU
	...
	function update_menu()
		if (menu.sub==0) then -- main
			...
			elseif (menu.id==1) then 
				if (btnp(4) or btnp(5)) then load_level_2()
				elseif (btnp(3)) then menu.id=2
				elseif (btnp(2)) then menu.id=0 end
			elseif (menu.id==2) then 
				if (btnp(4) or btnp(5)) then load_level_3()
				elseif (btnp(3)) then menu.id=3
				elseif (btnp(2)) then menu.id=1 end
			elseif (menu.id==3) then 
				if (btnp(4) or btnp(5)) then menu.sub=1 menu.id=0
				elseif (btnp(2)) then menu.id=2 end
			end
	...
	function draw_menu()
		...
		menu.t += 1/30
		if (menu.sub==0) then
			draw_clear_box(abs_box(44-menu.size, 80-menu.size, 82+menu.size, 114+menu.size))
			printui("level 1", 50, 84-menu.size*0.5, (menu.id==0 and 7 or 6))
			printui("level 2", 50, 92-menu.size*0.25, (menu.id==1 and 7 or 6))
			printui("level 3", 50, 100+menu.size*0.25, (menu.id==2 and 7 or 6))
			printui("extras", 52, 108+menu.size*0.5, (menu.id==3 and 7 or 6))
...
-->8 PLAYER
	...
	function draw_player()
		if (not player.dead) then
			local b = player.box
			if (player_col_down()) then
				if (player.can_move and btn(1) and not player.flip) then draw_anim(b.l, b.b-16, player.anim.move_right)
				elseif (player.can_move and btn(0) and player.flip) then draw_anim(b.l, b.b-16, player.anim.move_left)
				elseif (player.can_crouch and player.crouch) then 
...
--> PROPS
	...
	function update_fire_ball(fb) 
		if (hit_map(fb.box, 0b101) or hit_player(fb.box)) then
	...
	function rocket(x, y, pow)
		local rkt = create_prop(update_rocket, draw_rocket)
		rkt.box=box(x, y, 7, 6)
		rkt.pow = pow
		local dx = box_center(player.box).x-box_center(rkt.box).x
		local dy = box_center(player.box).y-box_center(rkt.box).y
		rkt.a = atan2(dx, dy)
	end
	...
	function update_rocket(rkt) 
		...
		local b=box_center(player.box)
		local da=atan2(b.x-rkt.box.l,b.y-rkt.box.t)
		rkt.a = da
		
		box_move(rkt.box, rkt.pow * cos(rkt.a), rkt.pow * sin(rkt.a))
	end
	...
	function ball_thrower(x, y, sx, sy, timers)
		local bt = create_prop(update_ball_thrower, draw_ball_thrower)
		bt.x=x*8 bt.y=y*8
		bt.sx=sx bt.sy=sy
		bt.timers = timers
	end
	...
	function draw_ball_thrower(bt)
		spr(96, bt.x, bt.y)
	end

	function rocket_thrower(x, y, pow, timers)
		local bt = create_prop(update_rocket_thrower, draw_rocket_thrower)
		bt.x=x bt.y=y
		bt.pow = pow
		bt.timers = timers
	end
	...
	function update_rocket_thrower(bt)
		if (#bt.timers==0 ) return 
		if (tick_timer(bt.timers[1])) then
			rocket(bt.x,bt.y, bt.pow)
		end
		if (bt.timers[1].n<=0) del(bt.timers, bt.timers[1])
	end

	function draw_rocket_thrower(bt)
		spr(112, bt.x, bt.y)
	end
...
--> UTILS
	...
	function new_timer(delay, rate, n)
		local timer = {delay=delay, rate=rate, n=n, t=0, dt=rate}
		add(timers, timer)
		return timer
	end
...
--> LEVELS
	...
	function load_level(spawn_x, spawn_y, update, draw)
		...
		props={}
		particles={}
		level = {spawn={x=spawn_x, y=spawn_y}, update=update, draw=draw, finished=false}
	...
	function load_level_1()
		if (progress.load == nil) progress.load=load_level_1
		load_level(60, 100, update_level_1, nil)
		level.t=0

		ball_thrower(7, 4, 0, 3, {new_timer(18, 2, 1000)})
		ball_thrower(8, 4, 0, 3, {new_timer(18, 2, 1000)})

		ball_thrower(31, 2, 2, 2, {new_timer(18, 2, 1000)})
		ball_thrower(51, 5, 0, 2, {new_timer(18, 2, 1000)})

		ball_thrower(77, 2, -1, 2, {new_timer(18, 2, 1000)})
		ball_thrower(78, 2, 0, 2, {new_timer(18, 2, 1000)})
		ball_thrower(79, 2, 1, 2, {new_timer(18, 2, 1000)})

		ball_thrower(94, 2, 0, 2, {new_timer(18, 2, 1000)})
		ball_thrower(95, 2, 0, 2, {new_timer(18, 2, 1000)})

		ball_thrower(108, 12, -2, 0, {new_timer(18, 2, 1000)})
		ball_thrower(108, 12, 2, 0, {new_timer(18, 2, 1000)})

		ball_thrower(109, 1, 0, 3, {new_timer(18, 2, 1000)})
		ball_thrower(110, 1, 0, 3, {new_timer(18, 2, 1000)})
	end

	function load_level_1_after_intro()
		load_level_1()
	...
	function update_level_1()
		if (not level.finished) then
			...
			if (level.t >= 3*30 and level.t <= 15*30) then
				local y1 = 12.8*8
				local y2 = 11.8*8
				local y3 = 10.8*8
				local x1 = 11
			...
			if (level.notice_wall_jump==nil and player.box.l >= 23*8) then
				...
				progress.load=load_level_1_after_intro
			end
		end
		if (not level.finished and player.box.l >= 123*8 ) then
			stars(box_center(player.box), "this cheese tastes good!")
			level.finished=true
			level.t=0
			player.speed.x=0
		...
		if (level.finished==true and level.t >=3*30) reboot()
	...
	function load_level_2()
		...
		load_level(4*8, 30*8, update_level_2, nil)

		ball_thrower(8, 24, -3, 3, {new_timer(2, 2, 1000)})
		ball_thrower(8, 24, 0, 3, {new_timer(2.5, 2, 1000)})
		ball_thrower(8, 24, 3, 3, {new_timer(3, 2, 1000)})

		ball_thrower(24, 21, -3, 3, {new_timer(2.5, 2, 1000)})
		ball_thrower(24, 21, 0, 3, {new_timer(2, 2, 1000)})
		ball_thrower(24, 21, 3, 3, {new_timer(2.5, 2, 1000)})

		ball_thrower(36, 25, 0, 3, {new_timer(2.5, 2, 1000)})


		ball_thrower(62, 25, 0, 3, {new_timer(2.5, .3, 1000)})
		ball_thrower(66, 25, 0, 3, {new_timer(2.8, .3, 1000)})
		ball_thrower(70, 25, 0, 3, {new_timer(3.1, .3, 1000)})
		ball_thrower(74, 25, 0, 3, {new_timer(3.4, .3, 1000)})
		ball_thrower(78, 25, 0, 3, {new_timer(3.7, .3, 1000)})

		ball_thrower(83, 33, 3, 0, {new_timer(3, 2, 1000)})
	...
	function update_level_2()
		if (not level.finished and player.box.l >=123*8) then
			stars(box_center(player.box), "this cheese is incredible!")
			level.finished=true
			level.t=0
			player.speed.x=0
			player.can_move=false
	...
	function load_level_3()
		...
		load_level(7*8, 51*8, update_level_3, nil)

			ball_thrower(17, 47, -4, 5, {new_timer(1.1, 1.3, 1000)})
		ball_thrower(17, 47, 0, 5, {new_timer(1.2, .8, 1000)})
		ball_thrower(17, 47, 4, 5, {new_timer(1.3, 1, 1000)})

		ball_thrower(27, 41, 3, 3, {new_timer(1.4, 3, 1000)})

		ball_thrower(39, 46, -4, 3, {new_timer(1, 3, 1000)})
		ball_thrower(39, 46, 0, 3, {new_timer(1, 3, 1000)})
		ball_thrower(39, 46, 4, 3, {new_timer(1, 3, 1000)})

		ball_thrower(59, 45, 0, 5, {new_timer(0, 2.5, 1000)})
		ball_thrower(60, 45, 0, 5, {new_timer(0, 2.5, 1000)})
		ball_thrower(61, 45, 0, 5, {new_timer(0, 2.5, 1000)})

		ball_thrower(69, 44, 0, 5, {new_timer(0, .3, 1000)})
		ball_thrower(77, 44, 0, 5, {new_timer(0, .3, 1000)})
		ball_thrower(85, 44, 0, 5, {new_timer(0, .3, 1000)})
		ball_thrower(93, 44, 0, 5, {new_timer(0, .3, 1000)})
	...
	function update_level_3()
		if (not level.finished and player.box.l >=123*8 and player.box.t >=51*8) then
			stars(box_center(player.box), "it is the best cheese ever!")
			level.finished=true
			level.t=0
			player.speed.x=0
			player.can_move=false
...

...

...
__label__

...

...

...

...

...

...

...

...

__music__
03 00010244
